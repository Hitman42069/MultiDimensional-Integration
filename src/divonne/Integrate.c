...
#define Out(f) Out2(f, &tot->phase[f])
    ...
    fprintf(fp,"Divonne Result:\n");
    fprintf(fp,"Number of Regions: %d\n",t->nregions);
    fprintf(fp,"Samples per Regions: %lld\n",t->samples[1].neff);
    fprintf(fp,"Number of Evaluations: %lld\n",t->neval);
    fprintf(fp,"Final Result: %.8lf +- %.8lf\tp = %.4lf\n",tot->integral,tot->sigsq,ChiSquare(tot->chisq, df));
    }
