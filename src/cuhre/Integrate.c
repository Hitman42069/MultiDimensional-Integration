...
static int Integrate(This *t, real *integral, real *error, real *prob)
{
  ...
  for( ; ; ) {
    ...
    fprintf(fp,"%.8lf +- " REAL "\tchisq %.8lf (%d df)\n",tot->avg,tot->err,tot->chisq,t->nregions - 1);}
      Print(out);
  ...
  for( tot = state->totals, comp = 0; tot < Tot; ++tot, ++comp ) {
    ...
    fprintf(fp,"Cuhre Result:\n");
    fprintf(fp,"Number of Regions: %d\n",t->nregions);
    fprintf(fp,"Number of Evaluations: %lld\n",t->neval);
    fprintf(fp,"Number of Failed Evaluations: %d\n",fail);
    fprintf(fp,"Final Result: %.8lf +- %.8lf\tchisq %.8lf\tp = %0.4lf\n",tot->avg,tot->err,tot->chisq,ChiSquare(tot->chisq, t->nregions - 1));
  }
